...
export default function SupplierInquiryPage() {
  ...
  // Respond form state
  const [sharedUtCode, setSharedUtCode] = useState("");
  ...
  useEffect(() => {
    ...
    setSharedUtCode(WH_MAP[active.site] ?? "");
    const init: Record<string, ItemRespond> = {};
    for (const item of detail.items) {
      init[item.id] = { status: "valid", replacement_pn: "", ut_note: "" };
    }
  ...
  const handleRespond = async () => {
    ...
    if (!sharedUtCode.trim()) {
      setToast({ msg: "Isi kode warehouse UT.", ok: false });
      return;
    }
    // Validate: every invalid item needs replacement_pn
    ...
    try {
      const responses = pendingItems.map(item => {
        const r = itemResponses[item.id] ?? { status: "valid", replacement_pn: "", ut_note: "" };
        return {
          item_id: item.id,
          status: r.status,
          replacement_pn: r.status === "invalid" ? (r.replacement_pn.trim() || null) : null,
          ut_site_code: sharedUtCode.trim() || null,
  ...
  const canSubmit = !!sharedUtCode.trim() && !submitting && pendingDetailItems.length > 0 &&
          ...
          <div className="bg-surface rounded-xl border border-[rgba(27,24,20,0.08)] overflow-hidden">
            ...
            {isLoading ? (
              ...
              items.map((inq: InquiryListItem) => {
                ...
                return (
                  <button key={inq.id} onClick={() => setActiveId(inq.id)}
            ...
            {active && detail ? (
              <>
                ...
                <div className="mb-4">
                  ...
                  <input
                    value={sharedUtCode}
                    onChange={e => setSharedUtCode(e.target.value.toUpperCase())}
                    placeholder="contoh: RTT"
                    maxLength={25}
                    className="w-full px-3 py-2.5 rounded-lg border text-[14px] font-mono font-bold tracking-widest text-ink bg-bg outline-none uppercase"
                    style={{ borderColor: sharedUtCode ? "#22C55E" : "rgba(27,24,20,0.12)" }}
                  />
                  <p className="mt-1 text-[10px] text-ink-3">
                    WH terdekat ke {active.site}:{" "}
                ...
                <div className="mb-4">
                  ...
                  <div className="rounded-lg border border-[rgba(27,24,20,0.08)] overflow-hidden divide-y divide-[rgba(27,24,20,0.06)]">
                    {detail.items.map((item, idx) => {
                      ...
                      if (!isPending) {
                        // Already responded — show read-only
                      ...
                      return (
                        <div key={item.id}
                          ...
                          {/* Replacement PN (shown only when invalid) */}
                          {isInvalid && (
                            <input
                              ...
                              style={{ borderColor: r.replacement_pn ? "#22C55E" : "#FCA5A5" }}
                            />
                          )}
