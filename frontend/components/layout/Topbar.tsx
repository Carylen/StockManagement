  ...
  return (
    <header className="sticky top-0 z-40 bg-bg border-b border-border flex items-center gap-4 px-6 py-4 flex-shrink-0">
      ...
      <button
        ...
        className="hidden md:flex items-center gap-2 px-3 py-2 bg-white ring-1 ring-border rounded-lg text-ink-3 text-sm w-60 hover:ring-border-strong transition-all"
      >
        <Search size={14} />
        <span className="flex-1 text-left">Cari PN, deskripsi…</span>
        <kbd className="text-[10px] font-mono bg-surface-alt px-1.5 py-0.5 rounded text-ink-2">⌘K</kbd>
      </button>

      {/* Notification */}
      <button className="relative w-9 h-9 rounded-lg bg-white ring-1 ring-border flex items-center justify-center text-ink-2 hover:ring-border-strong transition-all">
        <Bell size={16} />
      ...
      {/* User avatar */}
      {user && (
        <div className="flex items-center gap-2.5 pl-1.5 pr-3 py-1 rounded-full ring-1 ring-border bg-white">
