...
export function InquiryPulse({ data, loading }: Props) {
  const chartData = data?.map((item) => ({
    ...item,
    label: format(parseISO(item.date), "EEE", { locale: id }),
  }));

  return (
    <div className="bg-surface rounded-lg border border-[rgba(27,24,20,0.08)] p-4">
      <div className="mb-4">
        <h3 className="text-sm font-bold text-ink">Aktivitas Inquiry (7 Hari)</h3>
        <p className="text-xs text-ink-3 mt-0.5">Inquiry baru per hari</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="h-32 flex items-center justify-center">
          <p className="text-sm text-ink-3">Belum ada inquiry</p>
        </div>
      )}
    </div>
  );
